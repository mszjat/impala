...
public class AlterTableSetTblProperties extends AlterTableSetStmt {
  ...
  public void analyzeSerializationEncoding(Map<String, String> tblProperties)
    ...
    if (!(tbl instanceof FeFsTable)) {
      throw new AnalysisException(String.format("Property SERIALIZATION.ENCODING is only "
          + "supported on HDFS tables. Conflicting table: %1$s", tbl.getFullName()));
    }

    if (tbl instanceof FeIcebergTable) {
      throw new AnalysisException("Property SERIALIZATION.ENCODING is not supported " +
    ...
    if (partitionSet_ != null) {
      for (FeFsPartition partition : partitionSet_.getPartitions()) {
        if (!isLineDelimiterSameAsAscii(
          ...
          throw new AnalysisException(String.format(
              ...
              "Conflicting partition: %1$s. " +
              "Please refer to Jira ticket IMPALA-10319 for more info.",
      ...
      try {
        ...
        throw new AnalysisException(String.format(
            "Error parsing serialization.encoding for table: %1$s. ",
            table_.getFullName()));
      }
